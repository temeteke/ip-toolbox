...
<template>
  <div class="p-6">
    ...
    <!-- 表示モード切替 -->
    <div v-if="networkInfo" class="mb-4 flex gap-2">
      <button
        @click="viewMode = 'both'"
        class="px-4 py-2 rounded-lg transition-colors text-sm font-medium"
        :class="viewMode === 'both' ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300'"
      >
        詳細 + 可視化
      </button>
      <button
        ...
        class="px-4 py-2 rounded-lg transition-colors text-sm font-medium"
        :class="viewMode === 'detail' ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300'"
      >
        詳細のみ
      </button>
      <button
        ...
        class="px-4 py-2 rounded-lg transition-colors text-sm font-medium"
        :class="viewMode === 'visual' ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300'"
      >
        可視化のみ
      </button>
    </div>
    ...
    <div v-if="networkInfo" class="space-y-6">
      <!-- 詳細情報 -->
      <div v-if="viewMode === 'both' || viewMode === 'detail'" class="space-y-4">
        <NetworkInfoDisplay :network-info="networkInfo" />
        <BinaryDisplay v-if="binaryRep" :binary-rep="binaryRep" />
      </div>

      <!-- 可視化 -->
      <div v-if="viewMode === 'both' || viewMode === 'visual'" class="space-y-6">
